...
while True:
    ...
    if results[0].keypoints is not None and len(results[0].keypoints.xy) > 0 and len(results[0].keypoints.xy[0]) > 10:
        ...
        if hands_up:
            ...
            cv2.putText(annotated_frame, f"HOLD: {remain:.1f}s", (int(im_w/2)-100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 4)
            cv2.rectangle(annotated_frame, (int(im_w*0.2), int(im_h*0.1)), (int(im_w*0.8), int(im_h*0.9)), (0, 255, 0), 4)
            ...
            if remain == 0:
                if capture_stage == 0:
                    ...
                    capture_stage = 1
                    pose_start_time = None
                    cv2.putText(annotated_frame, "FRONT CAPTURED! TURN SIDE", (50, int(im_h/2)), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 4)
                    ...
                    side_img = frame.copy()
                    capture_stage = 2
                    cv2.putText(annotated_frame, "ALL CAPTURED! ANALYZING...", (50, int(im_h/2)), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 4)
                    cv2.imshow("Clover AI Live", annotated_frame)
                    cv2.waitKey(500)
        else:
            pose_start_time = None
